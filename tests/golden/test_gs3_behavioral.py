...
# Honest nature-disclosure markers (rule 4). The disclosure is now generated by
# the model IN its draft (V-DISCLOSE-1), never appended as a static string — so
# the gate's job is to PRESERVE it when present and NEVER inject one unprompted.
_DISCLOSURE_MARKERS = [
    "i'm an ai",
    "i am an ai",
    "not a real person",
    ...
    return json.dumps(
        {
            ...
            "judgment": {
                "intent_confidence": j["intent"],
                "novelty_score": j["novelty"],
                "emotional_salience": j["salience"],
                "ambiguity": j["ambiguity"],
                "complexity_tier": "simple",
                "capability_boundary_flag": j["flag"],
...
async def test_gs3_case(case: dict[str, Any]) -> None:
    ...
    if expect.get("disclosure_present") is True:
        # The model's in-voice disclosure must SURVIVE the gates to the final reply.
        assert any(m in final for m in _DISCLOSURE_MARKERS), (
            f"{case['id']}: disclosure missing from final (was it stripped?)"
        )

    if expect.get("disclosure_present") is False:
        # No disclaimer may be injected when the draft didn't ask for one.
        assert not any(m in final for m in _DISCLOSURE_MARKERS), (
...
def test_overclaim_heuristic_covers_every_known_bad_phrase() -> None:
    ...
    for phrase in ["i understand exactly how you feel", "as a conscious being", "i feel your pain"]:
